...
if($action == 'local') {
    ...
    $pagination = '';
    $pugin_cate_html = '<a class="btn btn-dark hidden-sm" target="_blank" href="https://bbs.oddfox.cn" title="奇狐网"><i class="icon-firefox"></i> 奇狐</a><a class="btn btn-dark hidden-sm" href="javascript:void(0);" onclick="javascript:location.reload();" title="刷新"><i class="icon-refresh"></i> 刷新</a><a class="btn btn-danger hidden-sm" href="https://bbs.oddfox.cn/sponsor.htm" target="_blank" title="赞助"><i class="icon-money"></i> 赞助</a>';    
    ...
    if(!empty($wd)){
        if(!empty($conf['url_rewrite_on'])){
            $pagination = pagination(url("plugin-$action-$cateid-{page}").'?wd='.xn_urlencode(trim($wd)), $total, $page, $pagesize);
        }else{
            $pagination = pagination(url("plugin-$action-$cateid-{page}").'&wd='.xn_urlencode(trim($wd)), $total, $page, $pagesize);
        }
    }
...
} elseif($action == 'synchro'){
    $json_path = APP_PATH.'log/plugin-all-4.json';
    $json_data = fox_write_plugin_json($json_path);
    if($json_data){
        cache_set('plugin_official_list', xn_json_decode($json_data), 3600);
        message(0, '已成功将远端插件列表同步至本地！');
    }else{
        message(-1, '远端插件列表同步至本地失败！');
    }
}
